...
--1.
 SELECT distinct state, d.YEAR 
  FROM FACT_TRANSACTIONS t, DIM_CUSTOMER c, DIM_DATE d, DIM_LOCATION L
  WHERE t.IDCustomer = c.IDCustomer AND
        t.Date = d.DATE AND
        t.IDLocation = l.IDLocation and
		d.YEAR between '2005' and GETDATE()
		order by d.YEAR ;


--2. 
select top 1 l.State, count(quantity) [count of phone]
from FACT_TRANSACTIONS t, DIM_LOCATION l, DIM_MODEL m, DIM_MANUFACTURER mn
where t.IDLocation = l.IDLocation and
		t.IDModel = m.IDModel and
		m.IDManufacturer =  mn.IDManufacturer and
...
--3.
select m.Model_Name, l.State,l.ZipCode, count(t.IDCustomer) [no. of transactons]
from FACT_TRANSACTIONS t, DIM_MODEL m, DIM_LOCATION l
where t.IDModel=m.IDModel and
		t.IDLocation=l.IDLocation 
...
from DIM_MODEL m
order by Unit_price 
------------------ taking for total price-------

select top 1 m.Model_Name, min(t.TotalPrice) [cheapest cell]
from FACT_TRANSACTIONS t, DIM_MODEL m
where t.IDModel=m.IDModel
group by m.Model_Name


--5
------------considering price as total price from transaction table----------------

select top 5 mn.Manufacturer_Name, m.Model_Name,  avg(t.TotalPrice) [avg price]
from FACT_TRANSACTIONS t, DIM_MODEL m, DIM_MANUFACTURER mn
where t.IDModel=m.IDModel and m.IDManufacturer=mn.IDManufacturer
group by m.Model_Name, mn.Manufacturer_Name
order by [avg price] desc


--6.
----considered total price from transaction table-----------

select c.Customer_Name,d.YEAR, avg(t.TotalPrice) [avg amont]
from FACT_TRANSACTIONS t, DIM_CUSTOMER c, DIM_DATE d
where t.IDCustomer=c.IDCustomer and t.Date=d.DATE and DATEPART(YEAR,d.DATE)=2009
group by c.Customer_Name,d.YEAR
having avg(t.TotalPrice)  > '500'


--7. 
select top 5 t.Quantity, d.YEAR
from FACT_TRANSACTIONS t,DIM_DATE d, DIM_MODEL m
where t.Date=d.DATE and t.IDModel=m.IDModel and YEAR(d.date) between 2008 and 2010
order by t.Quantity desc

--8.
----------------- i have considered sales as total price---------------------
----------------- i used row_number ---------------

select mn1.Manufacturer_name as Manufacturer_2009,mn2.Manufacturer_name as Manufacturer_2010
 from 
    ( SELECT Manufacturer_name,row_number() over(order by SUM (TotalPrice)) as row_no1
            ...
            group by Manufacturer_name )  mn1 
			inner join
    ( SELECT Manufacturer_name,row_number() over(order by SUM (TotalPrice)) as row_no2
            FROM Fact_Transactions t
            LEFT JOIN DIM_Model m ON t.IDModel = m.IDModel
            LEFT JOIN DIM_MANUFACTURER mn  ON mn.IDManufacturer =m.IDManufacturer
            Where DATEPART(Year,date)=2010 
            group by Manufacturer_name )  mn2 on mn1.row_no1=mn2.row_no2  and row_no1 in (1,2) and row_no2 in (1,2)


--9.

select  mn.Manufacturer_Name, d.YEAR
from FACT_TRANSACTIONS t, DIM_MANUFACTURER mn, DIM_MODEL m, DIM_DATE d
where t.Date=d.DATE and t.IDModel=m.IDModel and m.IDManufacturer=mn.IDManufacturer and DATEPART(year,d.DATE)!=2009 
group by mn.Manufacturer_Name, d.YEAR
except
select  mn.Manufacturer_Name, d.YEAR
from FACT_TRANSACTIONS t, DIM_MANUFACTURER mn, DIM_MODEL m, DIM_DATE d
where t.Date=d.DATE and t.IDModel=m.IDModel and m.IDManufacturer=mn.IDManufacturer and DATEPART(year,d.DATE) =2010
group by mn.Manufacturer_Name, d.YEAR
order by d.YEAR desc


--10.
	select top 100 a.*, ROUND((a.[avg spend]-a.[avg spend previous])*100/a.[avg spend previous],2) [% yoy change]
		from ( select [year]=year(t.date), c.Customer_Name, [avg quantity]=avg(t.quantity), [avg spend]=avg(t.totalprice),
			  [avg spend previous]=lag(avg(t.totalprice)) over(partition by c.customer_name order by year(t.date))
		from DIM_CUSTOMER c inner join FACT_TRANSACTIONS t on t.IDCustomer=c.IDCustomer group by c.customer_name , year(t.date)) a
